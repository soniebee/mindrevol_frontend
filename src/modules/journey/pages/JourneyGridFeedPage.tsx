...
import { ChevronDown, Check, Globe, MapPin, Grid3X3, Sparkles } from 'lucide-react';
  ...
  return (
    <MainLayout>
      <div className="flex flex-col h-[100dvh] bg-gradient-to-b from-[#F4EBE1] to-[#FFFFFF] dark:from-[#121212] dark:to-[#0A0A0A] w-full relative font-quicksand transition-colors duration-500">
        ...
        {/* Combobox Filter (Viên thuốc kính mờ) */}
        <div className="sticky top-0 z-30 flex justify-center pt-4 pb-4 bg-gradient-to-b from-[#F4EBE1] via-[#F4EBE1]/90 to-transparent dark:from-[#121212] dark:via-[#121212]/90 pointer-events-none">
          <div className="relative pointer-events-auto" ref={dropdownRef}>
            
            {/* Nút chính */}
            <button 
              onClick={() => setIsDropdownOpen(!isDropdownOpen)}
              className="flex items-center gap-2.5 px-4 py-2 bg-white/80 dark:bg-[#2B2A29]/80 backdrop-blur-md rounded-[20px] shadow-[0_8px_20px_rgba(0,0,0,0.06)] border border-white/50 dark:border-white/5 active:scale-95 transition-all"
            >
              {selectedUserId === null ? (
                <div className="w-7 h-7 rounded-[10px] bg-[#F4EBE1] dark:bg-[#1A1A1A] flex items-center justify-center border border-white/50 dark:border-transparent">
                ...
                <img 
                  ...
                  className="w-7 h-7 rounded-[10px] object-cover border border-[#F4EBE1] dark:border-[#3A3734]" 
                />
              )}
              
              <span className="text-[0.95rem] font-extrabold text-[#1A1A1A] dark:text-white truncate max-w-[120px]">
            ...
            {/* Menu thả xuống */}
            {isDropdownOpen && (
              <div className="absolute top-[calc(100%+8px)] w-[240px] bg-white/95 dark:bg-[#1A1A1A]/95 backdrop-blur-xl rounded-[24px] shadow-[0_16px_40px_rgba(0,0,0,0.12)] border border-[#D6CFC7]/50 dark:border-[#3A3734] py-2 left-1/2 -translate-x-1/2 origin-top animate-in fade-in zoom-in-95 duration-200 z-50">
        ...
        <div 
          className="flex-1 overflow-y-auto px-1.5 md:px-2 pt-1 pb-[100px] custom-scrollbar"
