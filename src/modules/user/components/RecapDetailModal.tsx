...
// [FIX] Import Checkin type for the callback
import { Checkin } from '@/modules/checkin/types';

interface Props {
    journeyId: string;
    // [FIX] Add journeyName to the interface (optional)
    journeyName?: string; 
    onClose: () => void;
    // [FIX] Define the callback type clearly
    ...
    return (
        <div className="fixed inset-0 z-[90] bg-black/95 animate-in fade-in duration-200 flex flex-col h-full">
            <div className="sticky top-0 z-10 bg-black/80 backdrop-blur-md p-4 flex justify-between items-center border-b border-white/10 max-w-7xl mx-auto w-full">
                <div>
                   <h2 className="text-xl font-bold text-white">{journeyName || "Journey memories"}</h2>
                   <p className="text-sm text-zinc-400">
                       {loading ? "Loading..." : `${checkins.length} memorable moments`}
                   </p>
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-4">
                <div className="max-w-7xl mx-auto">
                    {loading ? (
                        <div className="text-zinc-500 text-center py-20">Loading memories...</div>
                    ) : (
                        <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 pb-20">
                            {checkins.length > 0 ? checkins.map(checkin => (
                                ...
                                <div className="col-span-full text-zinc-500 text-center py-20 text-sm">No photos yet.</div>
                            )}
                        </div>
                    )}
                </div>
