...
import { X, User, Shield, FileText, LogOut, ChevronRight, Moon, Sun, Bell } from 'lucide-react';
...
export const SettingsModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  if (!isOpen && !showNotiSettings) return null;

  return createPortal(
      <div className={`fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 dark:bg-black/80 backdrop-blur-sm p-4 animate-in fade-in ${showNotiSettings ? 'pointer-events-none opacity-0' : ''}`}>
        <div className="w-full max-w-md bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transition-colors duration-300 relative">
